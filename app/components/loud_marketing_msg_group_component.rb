...
class LoudMarketingMsgGroupComponent < ApplicationComponent
  COLORS = {
    100 => { orange: 'text-black', rose: 'text-black', yellow: 'text-black' },
  ...
  def initialize(col:, color:, text:, color_gap:, animate: true)
    @col = col
    @color = color
    @text = text
    @animate = animate

    halved_color_range = Array.new(col) { |t| t.zero? ? 800 : t * color_gap + 100 }.slice(0..(col / ( color_gap / 100 ).floor))
